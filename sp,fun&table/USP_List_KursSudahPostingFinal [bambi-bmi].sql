...
BEGIN
	...
	IF(@status ='Y')
		BEGIN
			...
			SELECT No_Pls,No_Pli,NoPo,POTransacid,EntryDate,Note,supid,LastUserIDAccess,[bambi-bmi].[dbo].FunSumDetailPakingList_KURS_Temporary(No_Pls) as Totaldetail,
			Pib,Forwarder,Total,UserPosting,DatePosting,id_bl_awb
			FROM  [bambi-bmi].[dbo].POPAKINGLIST_KURS_Temporary  WHERE YEAR(EntryDate) =@tahun AND FlagPosting='Y'   ORDER BY EntryDate DESC  ;
		END
	 ELSE
		BEGIN
			INSERT INTO #temptess
			SELECT No_Pls,No_Pli,NoPo,POTransacid,EntryDate,Note,supid,LastUserIDAccess,[bambi-bmi].[dbo].FunSumDetailPakingList_KURS_Temporary(No_Pls) as Totaldetail,
			Pib,Forwarder,Total,UserPosting,DatePosting,id_bl_awb
			FROM  [bambi-bmi].[dbo].POPAKINGLIST_KURS_Temporary  WHERE YEAR(EntryDate) =@tahun AND LastUserIDAccess=@userid AND FlagPosting='Y'  ORDER BY EntryDate DESC  ;
		END
